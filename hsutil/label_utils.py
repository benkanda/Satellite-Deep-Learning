...
def from_one_hot(labels,label_dict=None):
    ...
    if labels.ndim == 2:
        for label in labels:
            if label_dict:
                conv_labels.append(inv_label_dict[np.argmax(label)])
            else:
                print(label)
                print(np.argmax(label))
                conv_labels.append(np.argmax(label))
        ...
        for i, row in enumerate(labels):
            ...
            for label in row:
                if label_dict:
                    conv_labels[i].append(inv_label_dict[np.argmax(label)])
                else:
                    conv_labels[i].append(np.argmax(label))
    else:
        raise ValueError("Labels array must be 2-dimensional or 3-dimensional.")
        
    return np.array(conv_labels)
