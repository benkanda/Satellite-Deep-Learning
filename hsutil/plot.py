...
from hsutil import label_utils
...
def class_plot(labels, class_labels=None, title=None, figure=1):
    ...
    classes = label_utils.get_classes(labels,as_array=True)
    maximum = max(classes)
    minimum = min(classes)
...
def evaluate_model(labels,patches,predictions,metrics,label_dict,class_labels=None,null_class=0):
    '''Could be good to generalise this later. At the moment, null_class has to be zero, so vmin and vmax prevent the color-
    bar picking it up.'''
    ...
    labels_cbar = fig_c.colorbar(labels_im, orientation="vertical", ticks=np.linspace(minimum+1.5,maximum+0.5,n_classes+1), ax=ax_c[0][0])
    ...
    classes = label_utils.get_classes(train_mask,as_array=True)
    maximum = max(classes)
    minimum = min(classes)
    n_classes = len(classes)
    if class_labels:
    ...
    classes = label_utils.get_classes(valid_mask,as_array=True)
    maximum = max(classes)
    minimum = min(classes)
    n_classes = len(classes)
    if class_labels:
    ...
    classes = label_utils.get_classes(predictions,as_array=True)
    maximum = max(classes)
    minimum = min(classes)
    n_classes = len(classes)
    if class_labels:
